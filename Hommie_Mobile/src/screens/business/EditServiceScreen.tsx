...
import { BusinessService, UpdateBusinessServiceDto } from '../../services/types/business.types';
...
export default function EditServiceScreen({ route, navigation }: EditServiceScreenProps) {
  const { service } = route.params;
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState<UpdateBusinessServiceDto>({
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true);
      await businessServiceApi.updateBusinessService(service.id, formData);
      Alert.alert(
        'Success',
