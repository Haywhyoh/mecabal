...
export class NigerianCarrierService {
   ...
   */
  detectCarrier(phoneNumber: string): {
    ...
    for (const [carrierName, carrierInfo] of this.carriers.entries()) {
      for (const prefix of carrierInfo.prefixes) {
        if (
          prefix.startsWith(shortPrefix) ||
          shortPrefix.startsWith(prefix.substring(0, 3))
        ) {
          return {
            carrier: carrierName as NigerianCarrier,
            info: carrierInfo,
   ...
   */
  getUSSDCodes(
    carrierName: NigerianCarrier,
  ): CarrierInfo['ussdCodes'] | undefined {
  ...
  getCarrierVerificationMessage(
    ...
    const messages = {
      en: {
        [NigerianCarrier.MTN]:
          'For MTN users, you can also verify by dialing *123*1# on your MTN line.',
        [NigerianCarrier.AIRTEL]:
          'For Airtel users, you can also verify by dialing *123*1# on your Airtel line.',
        [NigerianCarrier.GLO]:
          'For Glo users, you can also verify by dialing *777*1# on your Glo line.',
        [NigerianCarrier.NINE_MOBILE]:
          'For 9mobile users, you can also verify by dialing *200*1# on your 9mobile line.',
        [NigerianCarrier.UNKNOWN]:
          'Standard SMS verification will be used for your number.',
      },
    };

    return (
      (messages as Record<string, Record<string, string>>)[language]?.[carrierName] ||
