...
export class CreateJobApplicationsTable1760012952694 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createIndex(
      'job_applications',
      new Index('IDX_job_applications_job_id', ['job_id']),
    );

    await queryRunner.createIndex(
      'job_applications',
      new Index('IDX_job_applications_user_id', ['user_id']),
    );

    await queryRunner.createIndex(
      'job_applications',
      new Index('IDX_job_applications_status', ['status']),
    );

    await queryRunner.createIndex(
      'job_applications',
      new Index('IDX_job_applications_job_user', ['job_id', 'user_id']),
    );
